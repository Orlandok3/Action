    ...
    return new Promise((resolve) => {
      ...
      let url = {
              ...
              headers : { 'screen-inches' : `4.7`,
                          ...
                          'info-idfa' : `C6B1D4DF-7192-4D58-99E1-344E824B3474`,
                          'info-dt' : `phone_ad_sdk`,
                          'info-vcode' : `8c16a06addf822ad301e4d161a161056`,
                          'info-lon' : `0`,
                          'info-product' : `8936`,
                          'Connection' : `keep-alive`,
                          'Accept-Language' : `zh-cn`,
            ...
            $.post(url, async (err, resp, data) => {
              try {
                console.log("小说阅读数据： " + data);
                 //$.log(dkdbody)
                const result = JSON.parse(data)
                if(result.code == 0){
                        console.log('小说阅读任务回执:成功🌝 '+result.data.score)
                      }
                if(result.code == 1){
                        console.log('小说阅读任务回执:失败🚫 '+result.message)
                      }
                if(result.code == 10020){
                        console.log('小说阅读任务回执:失败🚫 '+result.message)
                      }
